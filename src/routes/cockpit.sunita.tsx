...
export const Route = createFileRoute("/cockpit/sunita")({
  head: () => ({ meta: [{ title: "Sunita Rao · Claims Lead · Bajaj General Insurance" }] }),
  component: Page,
});

const TEAL = "#0d9488";

function SLAGauge({ pct = 94.3 }: { pct?: number }) {
  ...
  return (
    <div>
      ...
      <div className="mt-1.5 text-[10.5px] text-sidebar-foreground/50">Target 98% · trending −1.2pp</div>
    </div>
  );
}
  ...
  return (
    <CockpitShell
      ...
      persona={{
        ...
        widgets: <SLAGauge pct={94.3} />,
      }}
      personaSlug="sunita"
      headerCenter="Sunita Rao · Claims Lead · BAGIC · Motor & Health · Pune Hub"
      contextStrip="BAGIC · Motor & Health Claims · Pune Hub — Monsoon surge period Jul–Sep (+340% motor claim volume expected)"
    >
      <MorningBriefing>
        Morning, Sunita. Today needs fast decisions on three fronts. 23 cashless pre-auth cases will breach the
        48-hour IRDAI mandate by 2pm — Prerna is overloaded at 68 cases but Karan has capacity to absorb 19 of
        them right now. Ravi Kumar is on Day 3 sick leave and his IRDAI cert expires in 22 days — he cannot
        legally process claims after that date without renewal. And Sunita Pillai's motor claim for Rajesh
        Pandey is now 52 days old with an IRDAI complaint filed — this one needs your direct call today, not
        a system action.
      </MorningBriefing>
      ...
      <ControlTower
        accent={TEAL}
        kpis={[
          { label: "SLA Compliance", value: "94.3%", sub: "Target 98% · ↓1.2pp yesterday", tone: "warning" },
          { label: "Cases in queue", value: "847", sub: "Active" },
          { label: "Breach risk today", value: "23 cases", sub: "By 2pm · IRDAI 48-hr mandate", tone: "critical" },
          { label: "Team present", value: "15 / 18", sub: "2 leave · 1 in training" },
          { label: "IRDAI cert risk", value: "2 analysts", sub: "Deepa 18d · Ravi 22d", tone: "critical" },
          { label: "Monsoon surge", value: "10 days away", sub: "Capacity gap 39%", tone: "warning" },
        ]}
      />

      <AIInsight
        accent={TEAL}
        title="Prerna Joshi is carrying 68 cases — she will cause tomorrow's SLA breaches if not redistributed now"
        body={`Queue analysis shows Prerna Joshi at 68 active cases — the highest on the team by 24 cases. Of her 68 cases, 14 are in the 36–47 hour age bracket — the breach zone. Prerna's case quality is excellent (98.2% accuracy rate) but her current load is physically unsustainable — she has worked 8 consecutive overtime days.
...
Recommended: Redistribute now. Do not wait for Prerna to raise the flag — she won't.`}
        confidence="92–96%"
        sources={["Case queue system", "Analyst capacity model", "SLA breach prediction engine"]}
      />

      <AIInsight
        accent={TEAL}
        title="Monsoon surge will overwhelm this team in 10 days — and the empanelment gap is already too late for standard hiring"
        body={`Jul–Sep motor claim surge historically runs at +340% vs current baseline for BAGIC Pune HAT. Current team at full capacity handles 100% of today's volume. The surge requires 2.4× capacity — meaning 12 additional analysts minimum.
...
Recommended: Trigger both in parallel today. Every day of delay shrinks the buffer.`}
        confidence="88–93%"
        sources={["BAGIC monsoon surge historical data", "Empanelment status tracker", "Capacity planning model"]}
      />

      <AIInsight
        accent={TEAL}
        title="Your team's overtime pattern is a retention risk that will peak exactly when you need them most"
        body={`4 analysts — Prerna, Suresh, Vijay, and Rekha — have worked 8+ consecutive overtime days. Combined, they carry 196 active cases and process 38% of daily volume.
...
Recommended: Mandatory 3-day rotation leave for all 4 before the surge starts — redistribute cases temporarily to rest of team. This costs 3 days of reduced capacity now to preserve full capacity through 90 days of surge. Also: raise a formal overtime recognition note to HRBP Neha — these 4 people are carrying the team and need to know you see it.`}
      ...
      <AgentActivity
        runs={[
          ...
          { agent: "IRDAI Cert Monitor", message: "Deepa (18d) + Ravi (22d) both unread system alerts · WhatsApp escalation recommended.", time: "02:44" },
          { agent: "Monsoon Surge Agent", message: "Capacity gap: 39% · 2 fast paths identified (agency + Mumbai deputation).", time: "03:30" },
          { agent: "Escalation Monitor", message: "Rajesh Pandey: Day 52 · IRDAI complaint response due EOD today.", time: "04:12" },
          { agent: "Overtime Risk Agent", message: "4 analysts at 8+ consecutive OT days · 3.1× resignation probability flagged.", time: "04:48" },
          { agent: "Capacity Match Agent", message: "Karan: 19 absorption units · Suresh: 12 units · covers all 23 at-risk cases.", time: "05:30" },
        ]}
      />
